...
o	Filter out NA’s from the birth year and hair colour columns
o	Change the birth year column type from numeric to character
o	Separate the hair_color column into one colour per row (hint: use the separate_rows function)
o	Convert the hair_color column to a factor
o	Create a function to multiply height by mass and then use map to apply it to the dataset to create a new column.
```{r}
starwars %>% 
  ...
  arrange(height) %>% 
  mutate(hair_color_new = factor(hair_color, levels = c("white","brown"))) %>% 
  arrange(hair_color) %>% 
  mutate(height_mass = function1(height, mass))
