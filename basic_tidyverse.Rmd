...
### Plotly
The plotly package can be used with ggplot to make the plots more interactive
```{r plotly, warning = FALSE}
ggplotly(ggplot(storms)+
  geom_point(aes(x = wind, y = pressure, colour = status)))
```

The ggplotly needs to wrap around the whole function, even when piping in data
```{r plotly_2, warning = FALSE}
ggplotly(storms %>% 
           filter(name == "Belle") %>% 
  ggplot()+
  geom_point(aes(x = wind, y = pressure, colour = status)))
...
Alternatively could save the plot as an object, then use within the ggplotly function
```{r plotly_3, warning = FALSE}
plot1 <- storms %>% 
           filter(name == "Belle") %>% 
  ggplot()+
  geom_point(aes(x = wind, y = pressure, colour = status))
...
The hover over text can be adjusted using the text argument
```{r plotly_4, warning = FALSE}
ggplotly(storms %>% 
           filter(name == "Belle") %>% 
  ggplot()+
  geom_point(aes(x = wind, y = pressure, colour = status, text = paste(name,
                                                                       "<br>Wind: ", wind,
...
Exercise:
For "diamonds", create a plot of "carat" vs "price" and colour by "cut".  Turn the plot into a ggplotly plot, an adjust the hover over text
```{r plotly_5, exercise = TRUE}
diamonds %>% 
  ggplot()+
  geom_point(aes(x = carat, y = price, colour = cut, text = paste())), 
  tooltip = c( )
```
...
ggplotly(diamonds %>% 
  ggplot()+
  geom_point(aes(x = carat, y = price, colour = cut, text = paste(cut,
                                                                  "<br>Clarity", clarity,
                                                                  "<br>Price: ", price,
                                                                  "<br>Carat: ", carat))), 
  tooltip = c("text"))
...
Exercise:
For "penguins", plot the average "body_mass_g" per "species" on a bar chart and colour by "species."  Turn the plot into a ggplotly, and adjust the hover over text
```{r plotly_6, exercise = TRUE}
...
ggplotly(penguins %>% 
  ...
  summarise(avg_body_mass = mean(body_mass_g, na.rm = T)) %>% 
  ggplot()+
  geom_col(aes(x = species, y = avg_body_mass, fill = species, text = paste(species,
                                                                            "<br>Average Body Mass: ", avg_body_mass))),
  tooltip = c("text"))
```
